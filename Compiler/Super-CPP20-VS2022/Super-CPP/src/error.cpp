...
namespace Super::Error
{
	...
	{
		{L"000000", L"不能使用该名作为宏名字"},
		{L"000010", L"不能使用该名解除宏定义或条件"},
		{L"000020", L"不能使用内置关键字作为宏条件"},
		{L"000030", L"导入文件路径必须是字符串"},
		{L"000040", L"使用了这个预处理指令但是他需要后续的代码"},
		{L"000050", L"不能使用变量名作为宏"},
		{L"000060", L"不能使用自定义类型名字作为宏"},
